...
{
	del_pid(current->pid);
	//printk("at my_exit_group around line 258\n");
	orig_exit_group(status);
	//printk("at my_exit_group around line 260\n");
}
//----------------------------------------------------------------
...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	// syscall_num refer to the syscall number
	int syscall_num = reg.ax;
	//check if current process's pid is being monitored or not, so we need to get info of the syscall_num
	if(table[syscall_num].monitored == 1){ //some pid are being monitored for the syscall_num
		if(check_pid_monitored(syscall_num,current->pid) == 1){ //current process is in the monitored list
			log_message(current->pid, (unsigned long int)syscall_num,reg.bx, reg.cx, reg.dx, reg.si, reg.di,reg.bp);
		}else{ //check_pid_monitored return 0, current process is not being monitored 
			return table[syscall_num].f(reg);
		}
	}else if(table[syscall_num].monitored == 2){//all processes are being monitered for syscall_num
		//printk("at interceptor around line 295\n");
		log_message(current->pid, (unsigned long int)syscall_num,reg.bx, reg.cx, reg.dx, reg.si, reg.di,reg.bp);
	}
	//call the original syscall_num after logging messages
	return table[syscall_num].f(reg);	
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	if(syscall > NR_syscalls || syscall == MY_CUSTOM_SYSCALL || pid < 0 || syscall < 0){ 
			return -EINVAL;
	
	}else{ 
	 	/*Intercept Command*/
		if(cmd == REQUEST_SYSCALL_INTERCEPT){
			if(current_uid() == 0){ /*Current user is root.*/
				if(table[syscall].intercepted == 1){ /*Syscal is intercepted already. Error returned.*/
					return -EBUSY;
				}else{ 	/*Interceot the syscall.*/
					
					/*Save the original syscall in meta-data table.*/
					table[syscall].f = sys_call_table[syscall];
					table[syscall].intercepted = 1;	
					/*set the sys_call_table writable*/
					set_addr_rw((unsigned long)sys_call_table);
					/*Synchronozation when changing the sys_call_table data.*/
					spin_lock(&sys_call_table_lock);
					/*Hijack the syscall to our function.*/
			...
			}else{ /*Not root, no permission to intercept */
				return -EPERM;
			}
		...
		/*Release Command.*/
			...
			if(current_uid() == 0){
				if(table[syscall].intercepted == 0){ /*Can't release syscall if it's not intercepted.*/
					return -EINVAL;
				}else{ /*Syscall have been intercepted, good to release.*/
					table[syscall].intercepted = 0;
					/*Changing the metadata table*/
					spin_lock(&my_table_lock);
					/*Delete all monitored pid from linked list.*/
					...
					/*Reset syscall to its original.*/
					sys_call_table[syscall] = table[syscall].f;
					
					spin_unlock(&sys_call_table_lock);
					set_addr_ro((unsigned long)sys_call_table);
				}
			}else{	/*not root, no permission to de-intercept*/
				return -EPERM;
			}
		...
		/*Monitor command*/
				...
				/*If syscall is not being intercepted yet, then monitoring is invalid*/
				...
				if(current_uid() == 0){
					...
					if(pid == 0){ 
						...
						spin_unlock(&my_table_lock);
					/*Monitor one process, check if pid exist.*/	
		       		}else if(pid_task(find_vpid(pid), PIDTYPE_PID) != NULL){/*Valid*/
		       			/*Check if current pid is monitored.*/
		       			if(check_pid_monitored(syscall, pid) == 1){ 
		       				return -EBUSY;
		       			}else{/*process pid is not yet being monitored for syscall*/

		       				/*Add pid to linked list of current syscall*/
		       				spin_lock(&my_table_lock);
			       			table[syscall].monitored = 1;       			
			       			add_pid_sysc(pid, syscall); //add pid to the syscall's monitoring list
			       			spin_unlock(&my_table_lock);
			       		}
					}else{/*No such pid exist in system.*/
						return -EINVAL;
					}
				/*User is not root, check if two pid has same owner.*/	
				}else if(pid != 0 && check_pids_same_owner(pid, current->pid) == 0){
					/*Check if pid exist.*/
					if(pid_task(find_vpid(pid), PIDTYPE_PID) != NULL){
						if(check_pid_monitored(syscall, pid) == 1){ /*check pid already monitored for syscall*/
		       				return -EBUSY;

		       			}else{/*Pid not yet monitored for syscall yet*/
		       				spin_lock(&my_table_lock);
			       			table[syscall].monitored = 1; /*just in case previously no pid is being monitored*/
			       			add_pid_sysc(pid, syscall); /*add to syscall's monitoring list*/
			       			spin_unlock(&my_table_lock);
			       		}
					}else{/*No such pid*/
						return -EINVAL;
				...
				}else{/*No permission for this user*/
					return -EPERM;
				}
			
		/*Stop monitor command.*/
		}else if(cmd == REQUEST_STOP_MONITORING){
				/*Syscall is not intercepted yet. Error.*/
			    if(table[syscall].intercepted == 0){
			    	return -EINVAL;
			    }
			    ...
			    /*User is root.*/
				if(current_uid() == 0){ 
					/*Stop monitor all processes.*/
					if(pid == 0){
						spin_lock(&my_table_lock);
						destroy_list(syscall);
						spin_unlock(&my_table_lock);

					/*Release one process. Check if pid exist.*/	
	       			}else if(pid_task(find_vpid(pid),PIDTYPE_PID) != NULL){ 
	       				/*Can't remove a pid when all processes are monitored.*/
	       				if(table[syscall].monitored == 2){ 
	       					return -EINVAL;
	       				}
	       				/*Can't stop when this pid is not monitored.*/
	       				if(check_pid_monitored(syscall,pid) == 0){ 
								return -EINVAL;
	       				}
	       				/*Pid is being monitored for this syscall*/
						spin_lock(&my_table_lock);
	       				del_pid_sysc(pid, syscall);/*Delete this pid from the monitored list of this syscall*/
	       				spin_unlock(&my_table_lock);


					}else{  /*pid is not valid*/
						return -EINVAL;
					}
				/*User is not root, cannot stop monitoring all processes. Check if two pids have same owner.*/
				}else if(pid !=0 && check_pids_same_owner(pid, current->pid) == 0){
					/*Check if this pid exist.*/
					if(pid_task(find_vpid(pid), PIDTYPE_PID) != NULL){
						...
						if(check_pid_monitored(syscall,pid) == 0){
	       				...
	       				/*pid is monitored for syscall*/
	       				/*Delete this pid from the monitored list of this syscall*/
						spin_lock(&my_table_lock);
				...
				}else{/*No permission for this user since not in root && two pids do not belong to the same owner*/
					return -EPERM;
				}
...
static int init_function(void) {
    ...
    /*store oringinal my_custom_call and exit_group function*/
	orig_custom_syscall = sys_call_table[MY_CUSTOM_SYSCALL];
	orig_exit_group = sys_call_table[__NR_exit_group];

	/*Synchronization for my_syscall_table*/
	set_addr_rw((unsigned long)sys_call_table);
	spin_lock(&sys_call_table_lock);
	/*replace custom and exit with our implemented ones*/
...
static void exit_function(void){        
	...
	set_addr_rw((unsigned long) sys_call_table);
	spin_lock(&sys_call_table_lock);
	/*restore oringinal exit and custom syscall*/
	sys_call_table[MY_CUSTOM_SYSCALL] = orig_custom_syscall;
	sys_call_table[__NR_exit_group] = orig_exit_group;

	/*restore all the syscall while unloading module*/
	for(j = 0; j < NR_syscalls; j++){
		if(table[j].f != NULL){
			sys_call_table[j] = table[j].f;
		}

		destroy_list(j); /*clean all information of table*/
	}
	spin_unlock(&sys_call_table_lock);
	set_addr_ro((unsigned long) sys_call_table);
}
