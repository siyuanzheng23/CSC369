...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { //no free page available
		...
		//calling coremap[frame].pte to get the info of pte that 'maps' to the physical frame
		pgtbl_entry_t *pte_evicted = coremap[frame].pte;
		
		//Checking whether physical memory frame has been written before swapping in 
		if(pte_evicted->frame & PG_DIRTY){ 
			evict_dirty_count ++ ;
			/*Set the new swap offset*/
			pte_evicted->swap_off = swap_pageout(frame, pte_evicted->swap_off);
			pte_evicted->frame |= PG_ONSWAP; //Set onSwap to 1
			pte_evicted->frame &= ~PG_VALID; //Set valid bit to 0, since not in frame anymore.
			pte_evicted->frame &= ~PG_DIRTY; //Set dirty bit to 0 after swapping in 

		/*The frame is not modified*/
		}else{

			pte_evicted->frame &= ~PG_VALID;
			pte_evicted->frame |= PG_ONSWAP; //Set onSwap to 1
			evict_clean_count ++;
		}
	...
	if(frame == -1){
	  printf("Error! No frame is evicted.\n");
	}
	...
	coremap[frame].pte = p;
	/*Update the created time for this frame.*/
	coremap[frame].howLong = howLongCount;
	//update arrival time counter.
	howLongCount ++;
...
char *find_physpage(addr_t vaddr, char type) {
	...
	/*page directory fault. Initialize this pde.*/
	if( !(pgdir[idx].pde & PG_VALID)){
	  	pgdir[idx] = init_second_level();
	}
	
	/*Clear the pde offser in order to locate the pte.*/
	pgtbl_entry_t *second_lvl =  (pgtbl_entry_t *) (pgdir[idx].pde & PAGE_MASK);
	p = second_lvl + PGTBL_INDEX(vaddr);
	
      

	/*if the entry is invalid(not holding the vpage) and not on swap, then this is the first reference 
	to the page and a (simulated) physical frame should be allocated and initialized.*/
	...
	/*If the entry is invalid and on swap, then a (simulated) physical frame
 	should be allocated and filled by reading the page data from swap.*/
		...
		int new_frame = allocate_frame(p);
		/*Swap the pte back frome swap file to frame.*/
	...
	/*Valid and not on swap*/
	}else if(((p->frame & PG_VALID)) && !(p->frame & PG_ONSWAP)){

		hit_count ++;		

	}else{/*this case caused error.*/
	  printf("valid: %d, onswap: %d", (int)(p->frame & PG_VALID), (int)(p->frame & PG_ONSWAP));
	  printf("\nERROR!\n");
	}

	/*Store virtual address to frame for opt algo.*/
	coremap[p->frame >> PAGE_SHIFT].vddr = vaddr & PAGE_MASK;
	...
	for (i=0; i < PTRS_PER_PGTBL; i++) {
		if (!(pgtbl[i].frame & PG_VALID) && 
			...
			if (pgtbl[i].frame & PG_VALID) {
				...
				//The line has warning, which is not allowed for 369's submission
